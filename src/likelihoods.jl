...
function loglike_parts(model, x::NamedTuple, spec::VisSpec)
...
function loglike_parts(model, x::NamedTuple, spec::VisAmpSpec)
	modvis = visibility(model, spec)
	μ = abs(modvis)
	σ = U.uncertainty(x.value)
...
function loglike_parts(model, x::NamedTuple, spec::ClosurePhaseSpec)
	modvis = visibility(model, spec)
	μ = angle(modvis)
	aval = angle(x.value)
...
function loglike_parts(model, x::NamedTuple, spec::ClosureAmpSpec)
	modvis = visibility(model, spec)
	μ = log(abs(modvis))
...
function loglike(model, x::NamedTuple, spec::VisSpec)
...
function loglike(model, x::NamedTuple, spec::VisAmpSpec)
	μ, σ, val = loglike_parts(model, x, spec)
...
function loglike(model, x::NamedTuple, spec::ClosurePhaseSpec)
	μ, σ, val = loglike_parts(model, x, spec)
...
function loglike(model, x::NamedTuple, spec::ClosureAmpSpec)
	μ, σ, val = loglike_parts(model, x, spec)
